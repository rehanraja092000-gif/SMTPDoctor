...
export default function AXFRChecker() {
  const [domain, setDomain] = useState("");
  const [result, setResult] = useState<any>(null);
  const [loading, setLoading] = useState(false);

  const checkAXFR = async () => {
    if (!domain) return;
    ...
    try {
      const res = await fetch(
        `/api/axfr-check?domain=${encodeURIComponent(domain)}`
      );
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="text-center">
          ...
          <p className="mt-6 text-2xl text-slate-600">
            Check whether DNS zone transfers (AXFR) are exposed.
          </p>
        </div>
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-slate-600">
                        Nameservers Found: {result.nameservers}
                      </p>
                    </div>
