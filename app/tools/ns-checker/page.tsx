...
export default function NSChecker() {
  ...
  const checkNS = async () => {
    ...
    try {
      const res = await fetch(
        `/api/ns-check?domain=${encodeURIComponent(domain)}`
      );
    ...
    <main className="min-h-screen bg-white px-6 py-20">
      <div className="mx-auto max-w-5xl text-center">
        ...
        <p className="mt-6 text-2xl text-slate-600">
          Check authoritative nameservers for any domain.
        </p>
        ...
        {result && (
          <div className="mt-14 rounded-3xl border p-8 text-left text-xl">
            ...
            <p className="mt-4">
              <strong>Nameservers:</strong> {result.count}
            </p>
